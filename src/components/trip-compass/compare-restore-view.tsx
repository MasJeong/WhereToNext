  ...
  return (
    <div className="compass-sheet rounded-[calc(var(--radius-card)-10px)] px-4 py-4 text-[var(--color-ink)] sm:px-5 sm:py-5">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2.5">
        ...
        <p className="text-xs leading-5 text-[var(--color-ink-soft)]">먼저 차이만 보고, 최종 후보가 보이면 상세 보기로 이어가세요.</p>
      </div>
    </div>
  );
}
...
export function CompareRestoreView({ data }: CompareRestoreViewProps) {
  ...
  return (
    <ExperienceShell
      eyebrow="결정 보드"
      title="저장한 후보를 한눈에 다시 비교해 보세요."
      intro="담아 둔 비교 보드를 그대로 복원해요. 중요한 차이만 보고 마지막 판단을 이어가면 돼요."
      capsule="저장된 비교 링크 · 2~4개 후보 비교 · 재계산 없음"
      headerAside={
      ...
      >
        ...
        <div className="mt-4 grid gap-2.5 sm:grid-cols-3">
          <article className="compass-desk rounded-[calc(var(--radius-card)-12px)] px-3.5 py-3.5">
            ...
            <p className="mt-1.5 text-sm font-semibold text-[var(--color-ink)]">저장한 후보만 다시 비교</p>
