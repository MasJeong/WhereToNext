...
export function CompareRestoreView({ data }: CompareRestoreViewProps) {
  ...
  return (
    <ExperienceShell
      eyebrow="결정 보드"
      title={`${data.columns.length}개 저장 후보를 마지막 판단용으로 가볍게 다시 정리한 비교 화면이에요.`}
      intro="비교 링크는 저장된 결과만 그대로 불러와요. 홈과 상세에서 마음에 든 여행을 담아 둔 뒤, 여기서는 차이만 빠르게 읽고 결정을 마무리하면 돼요."
      capsule="저장한 카드 2~4개 비교 · 저장 결과 그대로 복원 · 보조 판단 단계"
      headerAside={
        <div className="compass-sheet rounded-[calc(var(--radius-card)-10px)] px-4 py-4 text-[var(--color-ink)] sm:px-5 sm:py-5">
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--color-ink-soft)]">
            {data.columns.map((column) => column.card.destination.nameKo).join(" · ")}
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            <span className="compass-metric-pill rounded-full px-3 py-1.5 text-xs font-semibold">후보 {data.columns.length}곳</span>
            <span className="compass-metric-pill rounded-full px-3 py-1.5 text-xs font-semibold">저장된 비교 링크</span>
            <span className="compass-metric-pill rounded-full px-3 py-1.5 text-xs font-semibold">변형 없이 비교</span>
          </div>
        </div>
      }
    >
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 font-display text-[1.02rem] leading-tight tracking-[-0.03em] text-[var(--color-ink)] sm:text-[1.18rem]">
              같은 질문을 반복해서 보지 않도록, 핵심 차이만 남겨 마지막 판단을 빠르게 도와줘요.
            </h2>
            <p className="mt-2 text-sm leading-6 text-[var(--color-ink-soft)]">
              각 열은 저장한 여행지 하나이고, 각 행은 마지막에 다시 확인하게 되는 질문이에요. 차이만 보기로 중요한 판단 포인트만 남길 수도 있어요.
            </p>
          </div>
          <Link href="/" className="compass-action-secondary compass-soft-press rounded-full px-4 py-1.5 text-sm font-semibold">
            새 비교 보드 만들기
          </Link>
        </div>

        <div className="mt-4 flex flex-wrap gap-2">
          <span className="compass-metric-pill rounded-full px-3 py-1 text-[11px] font-semibold">저장한 카드만 비교</span>
          <span className="compass-metric-pill rounded-full px-3 py-1 text-[11px] font-semibold">추천 재계산 없음</span>
        </div>
