...
### 2026-04-05 - feature 최신 소스 일부가 dev에서 누락돼 빌드와 계정 흐름이 깨진 문제
- Symptoms: `dev`에서 `affiliateCategoryValues` 같은 export 누락으로 `next build`가 깨졌고, 계정 화면/세션/미래 여행/저장 추천 관련 타입과 selector도 feature 브랜치에서 작업했던 최신 상태와 어긋나 있었음.
- Cause: `feature/brand-login-refresh`의 마지막 작업 묶음이 squash/sync 과정에서 일부 파일만 남고, 핵심 계약/세션/계정/presentation 파일은 더 오래된 버전으로 남았음.
- Resolution: `feature` 마지막 시점으로 확인한 `31913ae` 기준에서 `contracts`, `auth`, `local-store`, `runtime/shell`, `test-ids`, `country-metadata`, `account-experience`, `presentation`을 복원해 최신 feature 계약과 화면 동작을 다시 맞췄음.
- Verification: `npm run build`, `npx vitest run tests/unit/domain/user-destination-history-image.spec.ts tests/unit/profile/future-trips.spec.ts tests/unit/auth/session-db-mode.spec.ts tests/unit/auth/session-storage-model.spec.ts tests/unit/ui/account-history-create-experience.spec.tsx`
- References: `src/lib/domain/contracts.ts`, `src/lib/auth.ts`, `src/lib/persistence/local-store.ts`, `src/lib/runtime/shell.ts`, `src/lib/test-ids.ts`, `src/lib/travel-support/country-metadata.ts`, `src/components/trip-compass/account-experience.tsx`, `src/lib/trip-compass/presentation.ts`

### 2026-04-05 - 홈 첫 화면과 유튜브 검색 힌트가 합의한 상태에서 벗어난 문제
- Symptoms: `dev` 최신 코드에서 홈 첫 화면이 단순 랜딩으로 보였고, `로그인 불필요`, `오늘 N명이 여행지를 찾았어요`, footer의 `떠나볼까?`와 `개인정보처리방침` 조합이 사라져 있었음. 함께 유튜브 검색 힌트에도 `한국인`, `korean`이 다시 남아 있었음.
- Cause: later squash/sync 과정에서 홈 랜딩과 shell footer가 단순화된 버전으로 남았고, social-video 검색 힌트 정리도 최신 `dev`에는 반영되지 않았음.
- Resolution: 홈 랜딩은 인기 여행지, 오늘 추천 수, `로그인 불필요` 문구와 sticky CTA가 있는 버전으로 복원했고, shell footer는 `떠나볼까?`와 `개인정보처리방침` 링크가 다시 보이게 되돌렸음. 함께 social-video 힌트와 stopword에서 `한국인`, `korean`을 제거했음.
- Verification: `npx vitest run tests/unit/social-video/service.spec.ts`, `npm run build`
- References: `src/components/trip-compass/home/landing-page.tsx`, `src/components/trip-compass/home-experience.tsx`, `src/components/trip-compass/experience-shell.tsx`, `src/lib/social-video/service.ts`

### 2026-03-27 - PR CI Playwright E2E를 현재 UI 계약과 브라우저 설치 상태에 맞춤
- Symptoms: 대표 추천 보강과 홈 퍼널 변경 후 PR의 `Playwright E2E` 체크가 다수 실패해 머지가 막혔음.
- Cause: 일부 e2e가 예전 랜딩 헤드라인, lead card copy, 저장 후 share-link 위치를 그대로 기대하고 있었고, CI는 `webkit` 프로젝트를 실행하면서도 브라우저 설치 단계에서 `chromium`만 설치하고 있었음.
- Resolution: 현재 UI 기준으로 smoke/recommendation/ios acquisition e2e 기대값을 업데이트했고, share-link는 형제 위치 가정 대신 실제 `href` 기준으로 찾도록 바꿨음. CI workflow는 `chromium webkit`을 함께 설치하도록 수정했음.
- Verification: `npm run lint`, `npm run test:unit`, `npm run build`
- References: `tests/e2e/smoke.spec.ts`, `tests/e2e/recommendation-flow.spec.ts`, `tests/e2e/ios-acquisition-flow.spec.ts`, `.github/workflows/ci.yml`
