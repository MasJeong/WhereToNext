...
### 2026-04-14 - 소셜 계정 재가입 뒤 앱 닉네임 대신 카카오 프로필명이 다시 보이던 문제
- 증상: 계정을 삭제한 뒤 카카오로 다시 로그인하면, 우측 상단 사용자명이 앱 기본 닉네임이 아니라 카카오 프로필명 `정지훈`으로 다시 보였음.
- 원인: `src/lib/provider-auth.ts`가 소셜 최초 가입 때 provider가 준 이름을 그대로 저장했고, 기존 소셜 계정 재로그인 때도 저장된 닉네임을 provider 이름으로 다시 덮어쓰고 있었음.
- 해결: 소셜 최초 가입은 항상 앱 기본 랜덤 닉네임을 생성하도록 바꾸고, 기존 소셜 계정 재로그인 때는 저장된 닉네임을 유지하도록 수정했음.
- 검증: `npx vitest run tests/unit/auth/provider-auth.spec.ts`, `npx vitest run tests/unit/auth/oauth-callback-shell.spec.ts`
- 참고 파일: `src/lib/provider-auth.ts`, `tests/unit/auth/provider-auth.spec.ts`

### 2026-04-14 - public 여행 이야기 시드가 적고 말투·닉네임·이미지 톤이 제각각이라 피드 신뢰감이 떨어지던 문제
- 증상: 로컬 Postgres의 공개 여행 이야기 수가 적었고, 일부 글은 테스트 데이터처럼 보여 `여행 이야기` 피드 전체 톤이 흔들렸음.
- 원인: 초기 샘플 몇 건만 수동으로 들어가 있었고, 닉네임 규칙·후기 말투·댓글 톤·이미지 품질을 피드 단위로 한 번에 맞춘 시드가 없었음.
- 해결: 기존 public 여행 이야기와 댓글을 정리하고, 실제 후기 플랫폼에서 자주 보이는 `짧은 실전 팁 + 동선 + 주의점` 톤으로 public 글 50건과 댓글 63건을 다시 넣었음. 작성자명은 `포동한너구리004` 같은 자동 닉네임 스타일로 통일했고, 각 글에는 목적지별 이미지 1장씩을 함께 저장했음. 삽입 중 `jsonb` 필드가 문자열로 들어가는 문제를 확인해 `tags`, `custom_tags`, `images`를 실제 JSON 배열 형태로 다시 저장했음.
- 검증: `psql`로 public 글 50건/이미지 포함 50건 확인, `curl http://127.0.0.1:4010/api/community` 응답에서 첫 페이지 20건과 닉네임/태그/이미지 shape 확인
- 참고 파일: `src/lib/community/service.ts`, `src/lib/db/schema.ts`, `docs/issue-resolution-log.md`

### 2026-04-14 - GitHub Actions에서 커뮤니티/여행 기록 스키마 변경 뒤 Drizzle migration 누락으로 배포 DB가 깨질 수 있던 문제
- 증상: `user_destination_history.visibility`와 `community_comments`를 사용하는 코드가 들어갔지만 `drizzle/0010`까지는 관련 enum, 컬럼, 테이블 생성 SQL이 없어 Postgres 마이그레이션 환경에서 CI 리뷰와 런타임 장애 위험이 남아 있었음.
- 원인: `src/lib/db/schema.ts` 변경 후 `npm run db:generate`로 신규 migration과 meta snapshot을 생성하지 않은 상태였음.
- 해결: `drizzle/0011_tough_saracen.sql`과 `drizzle/meta/0011_snapshot.json`, `drizzle/meta/_journal.json`을 추가해 `history_visibility` enum, `user_destination_history.visibility` 컬럼, `community_comments` 테이블/foreign key를 마이그레이션 체인에 반영했음.
- 검증: `npm run db:generate`, `npx vitest run tests/unit/auth/account-schema.spec.ts tests/unit/api/community-route.spec.ts tests/unit/api/community-comments-route.spec.ts`
- 참고 파일: `drizzle/0011_tough_saracen.sql`, `drizzle/meta/0011_snapshot.json`, `drizzle/meta/_journal.json`, `src/lib/db/schema.ts`

### 2026-04-14 - 공개 커뮤니티 댓글이 비로그인 사용자와 비공개 전환 뒤에도 계속 읽힐 수 있던 문제
- 증상: 공개 커뮤니티 피드는 로그인 없이 볼 수 있었고, 댓글 API도 별도 인증 없이 열려 있어 `historyId`를 아는 사용자가 댓글 내용을 직접 수집할 수 있었음. 공개 글을 나중에 비공개로 바꿔도 댓글 API는 부모 글 공개 상태를 다시 확인하지 않았음.
- 원인: `GET /api/community/[historyId]/comments`가 세션 검증과 부모 글 `visibility` 재검증 없이 댓글만 바로 조회하고 있었음. 공개 피드/댓글 조회에도 별도 rate limit이 없었음.
- 해결: 댓글 조회는 로그인 사용자만 가능하게 바꾸고, 부모 여행 이력이 현재도 `public`인지 `isPublicPostVisible()`로 다시 확인하도록 수정했음. 함께 공개 피드/댓글 읽기·쓰기 API에 IP 기준 rate limit 헤더와 429 응답을 추가했고, 게스트 UI는 댓글 본문을 요청하지 않고 로그인 안내만 보여주게 바꿨음.
- 검증: `npx vitest run tests/unit/ui/community-experience.spec.tsx tests/unit/api/community-route.spec.ts tests/unit/api/community-comments-route.spec.ts tests/unit/ui/community-detail-page.spec.tsx tests/unit/ui/community-detail-experience.spec.tsx`, `npm run build`
- 참고 파일: `src/app/api/community/route.ts`, `src/app/api/community/[historyId]/comments/route.ts`, `src/lib/community/service.ts`, `src/components/trip-compass/community-experience.tsx`

### 2026-04-14 - 저장소에 첫 진입할 때 프로젝트 성격과 파일 작성 기준을 한 번에 파악하기 어려운 문제
- 증상: `README.md`는 제품 소개 중심이고 `AGENTS.md`는 작업 규칙 중심이라, 처음 들어온 사람이 `이 프로젝트가 무엇인지`와 `새 파일에 어떤 내용을 넣어야 하는지`를 빠르게 파악하기 어려웠음.
- 원인: 방문자용 소개와 작업 규칙 사이를 이어 주는 짧은 온보딩 문서가 없었음.
- 해결: 루트에 `INIT.md`를 추가해 제품 성격, 핵심 구조, 시작 순서, 파일 유형별 작성 기준, 작업 전 체크 질문을 정리했고, `README.md`에서도 바로 진입할 수 있게 링크를 추가했음.
- 검증: `INIT.md`, `README.md` 내용 수동 검토 및 저장소 실제 경로/스크립트 대조
- 참고 파일: `INIT.md`, `README.md`

### 2026-04-14 - 여행 이야기 피드가 예쁘지만 추천 기준과 빠른 판단 근거가 약하던 문제
- 증상: `여행 이야기` 피드가 사진 중심으로는 정리돼 있었지만, 사용자가 `추천순`의 의미를 이해하기 어렵고 검색도 여행지명에만 묶여 있어 실제 탐색성이 낮았음.
- 원인: 정렬 기준 보조 설명이 없었고, 검색 대상이 `destinationName` 하나뿐이었으며, 카드 안에 `짧게 판단할 근거`가 구조화돼 있지 않았음.
- 해결: `추천순/최신순/별점` 정렬마다 설명 문구를 노출하고, 검색 범위를 여행지명·태그·후기 본문·작성자명까지 넓혔음. 함께 카드에 `빠르게 보면` 신호를 추가해 대표 태그, 사진 여부, 댓글 반응, 만족도, 최근성을 짧게 보여주도록 맞췄음.
- 검증: `npx vitest run tests/unit/ui/community-experience.spec.tsx`
- 참고 파일: `src/components/trip-compass/community-experience.tsx`, `tests/unit/ui/community-experience.spec.tsx`

### 2026-04-14 - 소셜 로그인 충돌 시 이전 로그인 수단을 알 수 없던 문제
- 증상: 같은 이메일로 이미 Google 등 특정 소셜 로그인 계정을 만든 사용자가 다른 수단으로 로그인하려고 하면, 인증 화면에는 `이전 방식으로 다시 로그인`하라는 일반 문구만 보여 어떤 수단을 써야 하는지 알기 어려웠음.
- 원인: `src/lib/provider-auth.ts`는 계정 충돌 여부만 반환했고, 실제로 기존에 연결된 provider 정보를 콜백 route와 인증 화면으로 전달하지 않았음.
- 해결: 계정 충돌 시 기존 providerId를 함께 반환하고, OAuth callback redirect가 `existingProvider` query를 붙여 인증 화면에서 `이미 Google로 로그인한 계정`처럼 구체적인 문구를 렌더링하도록 수정했음.
- 검증: `npx vitest run tests/unit/ui/auth-experience.spec.tsx`, `npx vitest run tests/unit/auth/oauth-callback-shell.spec.ts`
- 참고 파일: `src/lib/provider-auth.ts`, `src/app/api/auth/oauth/[provider]/callback/route.ts`, `src/components/trip-compass/auth-experience.tsx`

### 2026-04-14 - 로컬 Postgres 스키마가 커뮤니티 피드 코드와 어긋나 여행 이야기가 비어 보이던 문제
- 증상: `DATABASE_URL`을 쓰는 로컬 환경에서 `/api/community`가 정상 피드를 만들지 못했고, 공개 여행 이야기를 넣어도 화면에서 볼 수 없었음.
- 원인: 현재 코드의 `src/lib/community/service.ts`와 `src/lib/db/schema.ts`는 `user_destination_history.visibility`와 `community_comments` 테이블을 전제로 동작하는데, 로컬 Postgres에는 해당 column/table이 아직 없었음.
- 해결: 로컬 DB에 `history_visibility` enum, `user_destination_history.visibility`, `community_comments`를 idempotent SQL로 추가하고, 공개 글 3건과 댓글 6건의 샘플 데이터를 직접 삽입했음.
- 검증: `curl http://127.0.0.1:4010/api/community`, `curl http://127.0.0.1:4010/api/community/11111111-1111-4111-8111-111111111111/comments`, `http://127.0.0.1:4010/community` 응답 `200`
- 참고 파일: `src/lib/community/service.ts`, `src/lib/db/schema.ts`, `docs/issue-resolution-log.md`
...
### 2026-04-15 - Postgres 런타임 migration journal 불일치로 조회 경로가 막히는 문제 복구
- 증상: `DATABASE_URL`이 있는 런타임에서 기존 스키마가 이미 존재하는데 Drizzle migration journal이 비어 있거나 뒤처진 경우, `history_visibility` 같은 enum을 다시 만들다가 `createRuntimeDatabase()`가 실패했고 로그인/세션 조회와 계정 화면 진입까지 막혔음.
- 원인: Postgres 경로는 `migratePostgresJs()`를 그대로 호출하고 있었고, journal과 실제 스키마가 어긋난 환경에서는 중복 객체 생성 에러를 복구하지 못했음.
- 해결: Postgres 런타임 초기화에서 일반 migration 실패 시 `이미 존재하는 객체` 계열 에러만 식별해 best-effort SQL migration fallback으로 전환하도록 정리했음. 이 fallback은 기존 enum, 테이블, 컬럼, 스키마, 함수 중복 생성은 건너뛰고 나머지 migration/seed를 계속 적용해 조회 경로가 죽지 않게 함.
- 검증: `npx vitest run tests/unit/runtime/pglite-data-dir.spec.ts tests/unit/runtime/runtime-migration-error.spec.ts`, `npx eslint src/lib/db/runtime.ts tests/unit/runtime/runtime-migration-error.spec.ts`
- 참고 파일: `src/lib/db/runtime.ts`, `tests/unit/runtime/runtime-migration-error.spec.ts`

### 2026-04-14 - 여행 기록 step 탭 이동도 필수 입력 검증과 함께 막기
- 증상: 여행 기록 작성 화면에서 `다음` 버튼은 막혀도, 상단 step 탭을 눌러 `날짜`, `평점`, `태그`, `사진`, `메모`로 건너뛸 수 있어 필수 입력 흐름이 무너졌음.
- 원인: `moveNext()`에는 step 검증이 있었지만, `jumpToStep()`은 현재 step 유효성 확인 없이 인덱스만 바꾸고 있었음. 날짜/태그 입력 후 안내 에러를 즉시 지우는 처리도 일부 빠져 있었음.
- 해결: 현재 step 검증과 포커스 이동을 공통 helper로 묶고, 미래 step 탭 클릭도 같은 검증을 타도록 정리했음. 막힌 탭은 시각적으로 잠겨 보이게 하고, 클릭 시에는 현재 step에 머문 채 안내 문구와 포커스를 돌려주도록 바꿨음. 날짜/태그/커스텀 태그 입력 시 기존 에러가 바로 풀리도록 정리했음.
- 검증: `npx vitest run tests/unit/ui/account-history-create-experience.spec.tsx`
- 참고 파일: `src/components/trip-compass/account-history-create-experience.tsx`, `tests/unit/ui/account-history-create-experience.spec.tsx`

### 2026-04-14 - 플랫폼 역할 요약 문서 추가
- 증상: `Vercel`, `Cloudflare`, `Supabase`, `Neon`이 서로 무슨 역할인지 한 번에 구분하기 어려웠음.
- 원인: 기존 문서는 배포와 도메인 전략은 있었지만, 플랫폼 역할을 짧게 요약한 사용자용 문서가 없었음.
- 해결: `docs/platform-role-summary.md`와 `docs/platform-role-summary.rtf`를 추가해 배포, 도메인, DNS, DB, iOS 제출 플랫폼의 역할과 현재 프로젝트 기준 추천 조합을 한 장으로 정리했음.
- 검증: 문서 교차 검토
- 참고 파일: `docs/platform-role-summary.md`, `docs/platform-role-summary.rtf`
