...
### 2026-04-14 - 공개 지원 페이지와 심사용 연락처를 분리
- 증상: `/support`를 App Store Connect `Support URL`로 쓰려면 공개 페이지가 되는데, 여기에 심사용 전화번호까지 넣으면 웹과 앱 사용자에게 그대로 노출되는 상태였음.
- 원인: 공개 메타데이터용 `Support URL`과 비공개 심사용 `App Review Information` 연락처를 같은 정보 세트로 취급했음.
- 해결: `/support`에는 공개 가능한 `떠나볼까? 운영자`와 이메일만 두고, 실제 전화번호는 `App Review Information`에만 쓰도록 문서와 페이지를 수정했음.
- 검증: 지원 페이지 렌더링 확인, 제출 문서 교차 검토
- 참고 파일: `src/app/support/page.tsx`, `docs/ios-release-preflight.md`, `docs/ios-app-store-readiness-plan.md`, `docs/ios-app-store-connect-submission-pack.md`

### 2026-04-14 - iOS 제출용 운영 도메인과 배포 플랫폼 선택 기준 문서화
- 증상: `Vercel`, `Cloudflare Registrar`, 한글 도메인 fallback 등록처를 왜 선택하는지 근거가 대화에만 있고 문서에는 남아 있지 않았음.
- 원인: iOS 제출 준비 문서는 App Store Connect 입력값 중심이었고, 운영 도메인 확보 전략과 플랫폼 선택 이유를 별도 문서로 정리하지 않았음.
- 해결: `docs/ios-domain-hosting-strategy.md`를 추가해 `Vercel`을 운영 배포 1순위로 두는 이유, `Cloudflare Registrar`를 영문 도메인 1순위로 두는 이유, 한글 도메인 `IDN` fallback으로 `Namecheap`/`Porkbun`을 두는 이유와 실행 순서를 한국어로 정리했음.
- 검증: 공식 플랫폼 문서 대조, 기존 iOS 제출 문서와 교차 링크 확인
- 참고 파일: `docs/ios-domain-hosting-strategy.md`, `docs/ios-app-store-readiness-plan.md`, `docs/ios-at-home-release-checklist.md`, `docs/ios-app-store-connect-submission-pack.md`
