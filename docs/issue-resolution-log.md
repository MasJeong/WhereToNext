...
### 2026-04-02 - GitHub Actions unit 단계가 Vitest 워커 OOM으로 실패하던 문제
- 증상: PR 필수 체크 `Lint, Unit, Build`가 `tests/unit/smoke.spec.tsx` 종료 무렵 `JavaScript heap out of memory`, `Worker forks emitted error`와 함께 실패했음.
- 원인: GitHub Actions `ubuntu-latest`에서 전체 `vitest run`을 기본 Node 힙으로 실행하면서, 긴 단위 테스트 세트를 끝까지 유지하던 워커 프로세스가 메모리 한도에 걸렸음.
- 해결: `.github/workflows/ci.yml`의 unit 단계에 `NODE_OPTIONS=--max-old-space-size=6144`를 주입해 CI에서만 Node 힙을 늘려 전체 suite가 기본 메모리 한도에 막히지 않게 했음.
- 검증: 실패 run `23885761975` 로그 확인, `.github/workflows/ci.yml` 반영 확인
