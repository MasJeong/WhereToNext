...
| 분류 | 의미 |
|------|------|
| **Shell v1 supported** | Companion static shell app (`apps/ios-shell/`)에서 정적 내보내기(export)가 가능하고, Capacitor packaging 대상이 되는 라우트 |
| **Hosted-web only** | canonical hosted web (`src/app/`)에서만 동작하며, shell app에서는 정적 내보내기 대상이 아닌 라우트. Shell app에서 접근 시 hosted web으로 handover됨 |
| **Excluded** | Shell v1 범위에서 의도적으로 제외된 라우트. Shell app에서 접근 시 사용자에게 hosted web 사용을 안내 |

## 라우트 인벤토리 매트릭스

### Shell v1 Supported Routes
...
## Shell v1 URL Contract

Shell app에서 URL을 생성할 때는 다음 규칙을 따릅니다:
...
// - canonical URL은 hosted web으로 linking
```

## Architecture Decision Record

### Shell App Route Names vs Canonical URLs
...
2. Canonical URLs은 변경되지 않으며 hosted web의 source of truth로 유지
3. 사용자가 shell에서 snapshot 링크를 공유하면 → canonical `/s/[snapshotId]` URL 공유 (hosted web으로 유도)

### Static Export 호환성

Shell app에서 `output: "export"`가 가능한 이유:
...
현재 `src/app/**/page.tsx` 기준 top-level user-facing page routes는 다음 6개다.
...
위 6개는 모두 본 문서에서 정확히 한 번씩 분류된다. 별도 top-level `/history` page는 없지만, history 기능은 account/API surface에 묶여 있어 shell v1 범위에서 제외로 유지한다.
