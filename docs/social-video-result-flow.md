...
- 1위 카드에는 `메인 1개 + 서브 2개` 구조를 유지합니다.
- 추천 결과 2위, 3위 카드에는 경량 보조 영상 카드 1개씩을 추가로 붙일 수 있습니다.
- 백엔드는 YouTube `search.list`로 후보를 모으고, `videos.list`로 상세 메타데이터를 붙인 뒤 점수화합니다.
- 프런트는 `/api/social-video` 응답에서 `items`를 받아 1위 카드에는 큰 카드 1개와 작은 카드 2개를, 2위/3위 카드에는 작은 보조 카드 1개를 렌더합니다.
...
- `freshness`
  - 업로드 시점이 얼마나 최근인지 보되, 오래되지 않았는지만 확인하는 보조 축
- `engagementQuality`
  - 조회수 절대값을 가장 크게 보고, 게시 후 반응 속도와 반응 밀도를 함께 계산
- `durationPreference`
  - 추천 결과 상단에서 보기 좋은 영상 길이인지

### 5-2. 메인 선정 철학

메인은 단순히 `최신 영상`으로 고르지 않습니다.

이유는 아래와 같습니다.

- Shorts/짧은 영상은 최신성이 높아도 설명력은 낮을 수 있음
- 너무 오래된 대형 영상은 반응이 많아도 지금 추천 맥락과 안 맞을 수 있음
...
1. 메인 1개를 먼저 고릅니다.
2. 남은 후보 중 점수순으로 보조 2개를 채웁니다.
3. 가능하면 같은 `channelId`는 피해서 다양성을 유지합니다.
...
- 메인: 가장 먼저 볼 영상
- 서브 1: 메인을 보완하는 차순위 영상
- 서브 2: 비교용으로 함께 볼 차차순위 영상
...
3. 최대 3개까지 잘라 씁니다.
4. 1위 카드에는 `LeadSocialVideoPanel`이 메인 1개와 서브 2개를 크게 보여줍니다.
5. 2위, 3위 카드에는 `CompactSocialVideoPanel`이 가장 높은 점수의 보조 영상 1개를 작게 붙입니다.
