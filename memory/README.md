...
이 폴더는 **중요하거나 큰 작업의 공개 작업 기록**을 남기는 공간입니다.

숨김 폴더인 `.sisyphus/`가 에이전트 내부 계획과 메모를 담는다면, `memory/`는 사용자가 직접 확인할 수 있는 작업 기록을 담습니다.
...
- `verification.md`: 실행한 검증 명령, 결과, 남은 위험
...
- `docs/issue-resolution-log.md`는 반복 이슈와 해결책을 기록하는 문서이고, `memory/`는 개별 큰 작업의 공개 작업 이력을 남기는 용도입니다.
